...
def test_return_zscore_20d_zero_when_return_equals_rolling_mean():
    ...
    _compute_asset_features(df, asset_id=1, ticker="TEST")
    ...
    _compute_asset_features(df2, asset_id=1, ticker="T")
    ...
    df3 = pd.DataFrame(
        {
            ...
            "close": closes2,
            "volume": [1e6] * n2,
            "daily_return": flat_returns,
        }
    )
    _compute_asset_features(df3, asset_id=1, ticker="T")
...
def test_nan_feature_rows_are_dropped():
    ...
    with patch(
        "src.processing.feature_engineering.get_supabase", return_value=mock_supabase
    ):
