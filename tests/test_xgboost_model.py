...
def make_anomaly_row(volatility: float = 0.10, zscore: float = 0.0,
                     drawdown: float = -0.02, dd_acc: float = 0.0,
                     vol_z: float = 0.0) -> pd.DataFrame:
    return pd.DataFrame([{
        "asset_id": 1,
        "ticker": "SPY",
        "date": pd.Timestamp("2024-06-01"),
...
def test_write_anomaly_results_calls_update_not_insert():
    with patch("src.models.xgboost_model.get_supabase") as mock_get_supabase:
        ...
        results = [{"anomaly_flag": True, "anomaly_score": 0.75, "anomaly_type": "return_anomaly", "asset_id": 1, "ticker": "SPY"}]
...
def test_run_anomaly_detection_returns_success_dict_on_happy_path():
    ...
    with (
        ...
        patch("src.models.xgboost_model.build_features_for_date", return_value=today_df),
        ...
        mock_supabase.table.return_value.select.return_value.eq.return_value.limit.return_value.execute.return_value = portfolio_mock
...
def test_run_anomaly_detection_returns_failure_dict_on_exception():
    with (
        patch("src.models.xgboost_model.get_supabase") as mock_get_supabase,
        patch("src.models.xgboost_model.build_features", side_effect=RuntimeError("DB down")),
    ):
        mock_supabase = MagicMock()
        mock_get_supabase.return_value = mock_supabase
